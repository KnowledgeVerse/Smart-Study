...
async function renderBooks(list) {
  ...
  if (!list.length) {
    bookGrid.innerHTML = '<p style="text-align:center;color:#888;margin-top:2rem;">No books available.</p>';
    return;
  }
  for (const b of list) {
    ...
    card.innerHTML = `
      ...
      <div class="info">
        ...
        <div class="actions">
          <button class="download" onclick="downloadBook('${b.pdf}')" ${!pdfExists ? 'disabled style="opacity:0.5;"' : ''}>${pdfExists ? 'Download' : 'Not Available'}</button>
          <button class="read" onclick="readBook('${b.pdf}')" ${!pdfExists ? 'disabled style="opacity:0.5;"' : ''}>${pdfExists ? 'Read' : 'Not Available'}</button>
        </div>
        ${adminPanel}
      </div>`;
...
function attachAdminListeners() {
  document.querySelectorAll('.btnChangeCover').forEach(btn =>
    btn.addEventListener('click', e => {
      ...
      const fileInput = document.querySelector(`.newCoverFile[data-id="${id}"]`);
      fileInput.onchange = async ev => {
        const file = ev.target.files[0];
        if (!file) return;
        const formData = new FormData();
        formData.append('newCover', file);
        const res = await fetch(`http://localhost:3000/admin/book/${id}/cover`, {
          method: 'POST',
          body: formData,
          headers: { 'x-auth-token': 'admin-token-007' }
        });
        const result = await res.json();
        if (result.success) { alert('Cover updated'); location.reload(); }
  ...
  document.querySelectorAll('.btnDelete').forEach(btn =>
    btn.addEventListener('click', async e => {
      ...
      const result = await res.json();
      if (result.success) { alert('Deleted'); location.reload(); }
      else alert('Failed');
    })
  );
}
...
async function finalizeDraft(id) {
  ...
  const subject = document.querySelector(`.draftSubject[data-id="${id}"]`).value.trim();
  const name = document.querySelector(`.draftName[data-id="${id}"]`).value.trim();
...
function openUploadPage(event) {
  ...
  const uploadWindow = window.open('admin/upload.html', '_blank', 'width=800,height=600');
...
function openDraftUploadPage(event) {
  event.preventDefault();
  const draftWindow = window.open('admin/draft-upload.html', '_blank', 'width=800,height=600');
  if (!draftWindow || draftWindow.closed || typeof draftWindow.closed == 'undefined') {
    // Fallback for popup blockers
...
function openUploadPage() {
  ...
  for (let path of uploadPaths) {
    try {
      ...
      const newWindow = window.open(path, '_blank', 'width=900,height=700,scrollbars=yes,resizable=yes');
